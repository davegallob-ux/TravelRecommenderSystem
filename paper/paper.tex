...
Traditional recommender systems (TRSs) in the tourism domain are mainly divided into three categories~\cite{Sarkar2023, 8308293}:
\begin{itemize}
...
The prototype is designed as a single-page web application. The technical architecture consists of React, Vite for the build system, and vanilla CSS for layout and styling. Client-side math rendering is powered by KaTeX, a fast LaTeX math rendering library, integrated via a custom React wrapper. The system architecture is structured to support real-time user interaction.
...
\end{itemize}
This represents a non-compensatory filtering logic. A hotel that violates even a single constraint is immediately excluded from the results.
...
To ensure the mathematical correctness and stability of the recommender engine, the algorithmic logic was decoupled from the React user interface components and modularized into a standalone utility engine (\texttt{src/recommenderEngine.js}). A comprehensive automated unit testing suite (\texttt{run-tests.js}) consisting of exactly 60 distinct test cases (30 for Weighted MAUT and 30 for Constraint-Based Filtering) was developed to verify the engine's behavior under diverse input states and boundary conditions.

\subsubsection{MAUT Logic Verification (30 Cases)}
The 30 MAUT test cases evaluate normalization and scoring logic:
\begin{itemize}
  \item \textbf{Weight Boundary Conditions (5 cases)}: Verifies that equal weights generate uniform ranking criteria, zero weights are handled safely without division by zero, positive weights exceeding 10 are clamped to 10, negative weights are clamped to 0, and a single active weight evaluates to a clean $100\%$ match score for the top hotel.
  \item \textbf{Price relative cost normalization (5 cases)}: Tests that relative cost correctly maps the lowest price to utility 1.0, the highest price to utility 0.0, and intermediate prices proportionally. It checks that division by zero is avoided if all hotels share the same price, and out-of-bound prices are clamped.
  \item \textbf{Distance relative cost normalization (5 cases)}: Verifies closest hotel maps to utility 1.0, furthest hotel to utility 0.0, and out-of-bound distances are clamped.
  \item \textbf{Absolute Normalizations (5 cases)}: Asserts correct absolute utility mappings for stars (1-to-5 scale) and guest ratings/public transport scores (1-to-10 scale).
  \item \textbf{Weight Dominance and Flipped Rankings (5 cases)}: Tests that highly weighted attributes dominate ranking and changing weights correctly flips ranking order.
  \item \textbf{Robustness and Sorting (5 cases)}: Verifies empty list inputs, null inputs, missing weight keys, strict descending sorting order of match scores, and correct formatting of breakdown scores.
\end{itemize}

\subsubsection{Constraint-Based Filtering Verification (30 Cases)}
The 30 filtering test cases evaluate the logical conjunction boundaries:
\begin{itemize}
  \item \textbf{Individual Attribute Constraints (25 cases)}: Tests price, location distance, star ratings, guest review scores, and public transport access scores separately. In each category, 5 tests verify strict exclusions, inclusive boundaries, extremely tight constraints yielding empty sets, extremely loose constraints including all items, and default constraint value fallbacks if omitted.
  \item \textbf{Conjunctive Combination Rules (5 cases)}: Verifies that multiple active constraints are evaluated as a logical conjunction (non-compensatory logic). Asserts that an excellent score in one attribute (e.g. 5 stars) cannot compensate for a failure in another attribute (e.g. exceeding the maximum price). Tests empty input lists, null inputs, and empty constraint configurations.
\end{itemize}
The entire test suite executes synchronously against a representative hotel database mock, confirming that all 60 test cases pass.
...
To prevent initial cognitive overload and hide transparency panels before the user has chosen an algorithm, the landing page is designed as an interactive selection wizard. Initially, the algorithm selection state is set to \texttt{null}, hiding the recommendation results, importance sliders, constraint inputs, and transparency boards. Instead, a clean strategy selection interface is displayed. This screen presents two prominent selection cards side-by-side:
\begin{itemize}
...
\end{itemize}
This wizard forces the user to make an explicit choice, making the algorithmic concept transparent from the very beginning. Once a strategy is chosen, the main interface is loaded. The user can switch the strategy at any time using a persistent "Switch Algorithm" back button or toggle buttons at the top of the dashboard.
