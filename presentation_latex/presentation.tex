...
\begin{frame}
    ...
    \begin{itemize}
        \item \textbf{The Recommender "Black Box":}
            \begin{itemize}
                \item Most modern recommender systems (RS) hide their logic (e.g. neural network rankings)~\cite{adomavicius2005toward, BORRAS20147370}.
        ...
        \item \textbf{Regulatory Requirements: EU Digital Services Act (DSA)~\cite{eudsa2022}:}
            ...
            \end{itemize}
        \item \textbf{Project Goal:} Build an interactive, transparent \& DSA-compliant tourism recommender system prototype.
    \end{itemize}
\end{frame}
    ...
    \begin{columns}
        \begin{column}{0.53\textwidth}
            \begin{itemize}
                \item \textbf{Compensatory Utility Scoring~\cite{schmitt2002maut}:}
                    \begin{itemize}
                        \item High score in one attribute (e.g. location) can compensate for low performance in another (e.g. price).
                        \item Profiling-free (DSA-compliant)
                    \end{itemize}
                \item \textbf{Weights on a 0-10 Scale:}
                    \begin{itemize}
                        \item Range: $0$ (completely unimportant) to $10$ (most important), changed after feedback in Milestone 3.
        ...
        \begin{column}{0.43\textwidth}
            \begin{center}
                ...
                \begin{figure}
                    \tiny\caption{Transparency Board Info}
                \end{figure}
            \end{center}
        \end{column}
    \end{columns}
...
\begin{frame}
    ...
    \begin{itemize}
        ...
        \item \textbf{Absolute Normalization (Benefit attributes: Stars, Rating, Transit):}
            \begin{itemize}
                \item Normalizes value $v$ on fixed scale bounds as they are known:
            \end{itemize}
            \begin{equation*}
        ...
        \begin{column}{0.48\textwidth}
            \begin{itemize}
                \item \textbf{Non-Compensatory Logic~\cite{ricci2011travel}:}
                    \begin{itemize}
                        \item User defines absolute thresholds (e.g. maximum price, minimum stars) $\Rightarrow$ Hotel must satisfy all constraints.
                        \item Over-performing in one area cannot compensate for a single constraint violation.
                    \end{itemize}
                \item \textbf{DSA-Compliance:}
                    \begin{itemize}
                        \item Transparent: Easy-to-understand inclusion/exclusion criteria.
...
\begin{frame}
    \frametitle{Improvement upon Milestone Feedback}
    \vspace{1em}
    \begin{columns}
        \begin{column}{0.48\textwidth}
            ...
            \begin{itemize}
                ...
                \item Entering Point showed overloaded transparency board before seeing the sliders.
...
\begin{frame}
    ...
    \begin{itemize}
        \item \textbf{The Explanation Dilemma~\cite{tintarev2012evaluating}:}
            ...
            \end{itemize}
        \item \textbf{Tiered Disclosure Features:}
            \begin{itemize}
                \item \textbf{Novices:} Brief text explanations focusing on the meaning of user controls. Satisfies general transparency.
                \item \textbf{Experts (KaTeX Formulas):} Button shows raw equations, especially good for transparency audits and regulators.
                \item \textbf{Interested Users:} Collapsible Step-by-step breakdown $\Rightarrow$ Explains individual recommendations, boosting user trust.
            \end{itemize}
    \end{itemize}
\end{frame}
...
\begin{frame}
    ...
    \begin{itemize}
        \item \textbf{Hypotheses:}
            \begin{itemize}
                \item \textbf{H1 (Perceived Control)}
                \item \textbf{H2 (System Trust)}
                \item \textbf{H3 (DSA Compliance Impact)}
            \end{itemize}
